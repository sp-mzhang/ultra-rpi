...
class IoTClient:
    ...
    def _resolve_cert(self, *names: str) -> str:
        '''Return the first existing cert path from *names*.

        Prefers device certs (generated by provisioning) but
        falls back to claim certs when device certs don't
        exist yet.
        '''
        for name in names:
            path = os.path.join(self._cert_dir, name)
            if os.path.isfile(path):
                return path
        return os.path.join(self._cert_dir, names[0])
    ...
    def connect(self) -> bool:
        ...
        cert_path = self._resolve_cert(
            'device.pem.crt', 'claim.cert.pem',
        )
        key_path = self._resolve_cert(
            'device.pem.key', 'claim.private.key',
        )
        ca_path = self._resolve_cert(
